...
*/
/*property
    addClass, append, biopic, blog, certificate, children, concat, contacts,
    dates, degree, description, display, email, employer, facebook, forEach,
    github, html, images, institute, jobs, length, location, majors, mobile,
    name, prepend, projects, replace, role, schools, skills, slice, split,
    title, toLowerCase, toUpperCase, trainingCourses, trim, twitter, url,
    welcomeMessage
*/
var bio = {
    "name": "MichAel ZhAng",
    ...
    "contacts": {
        "location": "City of Ningbo 315100,<br>Zhejiang Provience (China)",
...
function placeholderImage(title, width = 120, height = 120, txtsize = 12) {
...
bio.display = function () {
    ...
    var formattedLocation = HTMLemail.replace(/%data%/g, bio.contacts.location);
...
education.display = function () {
    ...
    education.trainingCourses.forEach(function (trainingCourse) {
        var formattedTrainingInfo = HTMLtrainingInfo.replace("%dates%", trainingCourse.dates);
        formattedTrainingInfo = formattedTrainingInfo.replace("%cert%", trainingCourse.certificate);
        formattedTrainingInfo = formattedTrainingInfo.replace("%title%", trainingCourse.title);
        formattedTrainingInfo = formattedTrainingInfo.replace("%url%", trainingCourse.url);
        formattedTrainingInfo = formattedTrainingInfo.replace("%institute%", trainingCourse.institute);
...
$("title").html(inFullname.concat(" - ", bio.role));
bio.display();
work.display();
projects.display();
education.display();
